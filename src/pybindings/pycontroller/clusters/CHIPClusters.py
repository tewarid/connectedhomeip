...
import ctypes
from  ChipCaller.Caller import Caller
...
class ChipClusters:
    ...
    def SendCommand(self, device: ctypes.c_void_p, cluster: str, command: str, endpoint: int, groupid: int, args, imEnabled):
        ...
        funcCaller = self._ChipStackCaller.Call if imEnabled else self._ChipStackCaller.CallAsync
    ...
    def InitLib(self, chipLib):
        ...
        def HandleSuccess():
            self._ChipStackCaller.callbackRes = 0
            self._ChipStackCaller.completeEvent.set()
        def HandleFailure(status):
            self._ChipStackCaller.callbackRes = status
            self._ChipStackCaller.completeEvent.set()
